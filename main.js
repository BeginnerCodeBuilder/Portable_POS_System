...
const CustomersRoutes = require('./brain/customers.routes');
const BillersRoutes = require('./brain/billers.routes'); // ✅ Billers module
...
app.whenReady().then(() => {
  ...
  // 🧠 Customers IPC
ipcMain.handle("customers:get-all", (_, filters) => CustomersRoutes.getAll(filters));
ipcMain.handle("customers:add", (_, data) => CustomersRoutes.add(data));
ipcMain.handle("customers:update", (_, data) => CustomersRoutes.update(data));
ipcMain.handle("customers:get-logs", (_, id) => CustomersRoutes.getLogs(id));
ipcMain.handle("customers:import", (_, filePath) => CustomersRoutes.import(filePath));
ipcMain.handle("customers:export", () => CustomersRoutes.export());
ipcMain.handle("customers:get-rewards-summary", (_, id) => CustomersRoutes.getRewardsSummary(id)); // 🆕 For rewards equivalent
ipcMain.handle("customers:get-one", (_, id) => CustomersRoutes.getOne(id));


  ipcMain.handle("customers:pickFile", async () => {
  ...
  // 🧠 Billers (Maintenance Companies) IPC
ipcMain.handle("billers:get-all", (_, filters) => BillersRoutes.getAll(filters));
ipcMain.handle("billers:add", (_, data) => BillersRoutes.add(data));
ipcMain.handle("billers:update", (_, data) => BillersRoutes.update(data));
ipcMain.handle("billers:get-one", (_, id) => BillersRoutes.getOne(id));
ipcMain.handle("billers:get-logs", (_, id) => BillersRoutes.getLogs(id));
ipcMain.handle("billers:get-contacts", (_, billerId) => BillersRoutes.getContacts(billerId));
ipcMain.handle("billers:update-contacts", (_, { biller_id, contacts }) => BillersRoutes.updateContacts(biller_id, contacts));
ipcMain.handle("billers:import", (_, filePath) => BillersRoutes.import(filePath));
ipcMain.handle("billers:export", () => BillersRoutes.export());

ipcMain.handle("billers:pickFile", async () => {
  const result = await dialog.showOpenDialog({
    properties: ["openFile"],
